...
class DisAllowRole extends Command {
    ...
    async run(message, args, client) {
        ...
        data.allowed.pop(target.id);
        data.save();

        client.logger.log(`Role ${data.roleid} dis allowed to be given by ${target.id} (ran by ${message.author.id})`);
        message.channel.send("Role permissions updated.");
    };

}

module.exports = DisAllowRole